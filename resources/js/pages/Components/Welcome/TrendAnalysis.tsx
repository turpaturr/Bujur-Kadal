...
import { TrendingUp, Activity, Flame, ShieldCheck, Wind, Info } from '@/pages/Components/Welcome/Icons';
...
export const KARHUTLA_CHART_DATA: ChartDataPoint[] = [
    { week: "M1 Agt", hotspots: 140,  ispa: 1450,  burnedArea: 1820,  pm25: 45 },
    { week: "M2 Agt", hotspots: 380,  ispa: 2600,  burnedArea: 4400,  pm25: 88 },
    { week: "M3 Agt", hotspots: 720,  ispa: 4300,  burnedArea: 10800, pm25: 165 },
    { week: "M4 Agt", hotspots: 1120, ispa: 6800,  burnedArea: 21500, pm25: 280 },
    { week: "M1 Sep", hotspots: 1380, ispa: 9200,  burnedArea: 36200, pm25: 395 },
    { week: "M2 Sep", hotspots: 1210, ispa: 10400, burnedArea: 52400, pm25: 460 },
];
...
export default function TrendAnalysis() {
    ...
    return (
        <section id="analisis" className="mt-20 sm:mt-24">
            {/* Header Section */}
            <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
                <div className="rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-4 sm:p-5 shadow-xs flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-semibold tracking-wider text-[#2FA084]">
                            Puncak Kasus ISPA
                        </span>
                        <div className="h-7 w-7 rounded-lg bg-[#2FA084]/15 text-[#2FA084] flex items-center justify-center">
                            <Activity className="h-4 w-4" />
                        </div>
                    </div>
                    <div className="font-heading font-serif text-2xl sm:text-3xl font-bold text-[#1F6F5F] mt-2 tabular-nums">
                        10.400+
                    </div>
                    <span className="text-[11px] text-muted-foreground mt-1 block">Kasus mingguan (M2 Sep)</span>
                </div>

                <div className="rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-4 sm:p-5 shadow-xs flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-semibold tracking-wider text-[#1F6F5F]">
                            Titik Panas Terdeteksi
                        </span>
                        <div className="h-7 w-7 rounded-lg bg-[#1F6F5F]/15 text-[#1F6F5F] flex items-center justify-center">
                    ...
                    <span className="text-[11px] text-muted-foreground mt-1 block">Hotspot NASA FIRMS</span>
                </div>

                <div className="rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-4 sm:p-5 shadow-xs flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] uppercase font-semibold tracking-wider text-amber-700">
                            Total Lahan Hangus
                        </span>
                        <div className="h-7 w-7 rounded-lg bg-amber-500/15 text-amber-700 flex items-center justify-center">
                            <ShieldCheck className="h-4 w-4" />
                        </div>
                    </div>
                    <div className="font-heading font-serif text-2xl sm:text-3xl font-bold text-amber-800 mt-2 tabular-nums">
                        52.400 Ha
                    </div>
                    <span className="text-[11px] text-muted-foreground mt-1 block">Gambut &amp; hutan terbakar</span>
                </div>
                ...
                <div className="rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-4 sm:p-5 shadow-xs flex flex-col justify-between">
                    ...
                    <span className="text-[11px] text-rose-600 font-medium mt-1 block">Kategori Sangat Berbahaya</span>
                </div>
            </div>

            {/* Interactive SVG Chart Container */}
            <div className="rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-4 sm:p-6 shadow-xs">
                {/* Header di dalam Card: Judul & Filter Toggles */}
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 border-b border-[#EEEEEE] pb-4 mb-4">
                    <div>
                        <span className="text-xs font-semibold text-[#1F6F5F] block">
                            Kurva Tren &amp; Korelasi Mingguan
                        </span>
                        <span className="text-[11px] text-muted-foreground">
                            Pilih metrik di bawah untuk menyaring grafik interaktif
                        </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 text-xs">
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all cursor-pointer ${
                                showIspa
                                    ? 'bg-[#2FA084]/15 border-[#2FA084] text-[#1F6F5F] font-semibold shadow-2xs'
                                    : 'border-[#EEEEEE] text-muted-foreground opacity-50 hover:opacity-80'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all cursor-pointer ${
                                showHotspots
                                    ? 'bg-[#1F6F5F]/15 border-[#1F6F5F] text-[#1F6F5F] font-semibold shadow-2xs'
                                    : 'border-[#EEEEEE] text-muted-foreground opacity-50 hover:opacity-80'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all cursor-pointer ${
                                showBurnedArea
                                    ? 'bg-amber-500/15 border-amber-600 text-amber-800 font-semibold shadow-2xs'
                                    : 'border-[#EEEEEE] text-muted-foreground opacity-50 hover:opacity-80'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all cursor-pointer ${
                                showPm25
                                    ? 'bg-[#6FCF97]/25 border-[#2FA084] text-[#1F6F5F] font-semibold shadow-2xs'
                                    : 'border-[#EEEEEE] text-muted-foreground opacity-50 hover:opacity-80'
                            }`}
                        >
                ...
                {/* Relative Chart Area with Confined In-Canvas Tooltip */}
                <div className="relative w-full h-[300px] sm:h-[340px]">
                    {/* Tooltip Overlay: Disimpan di dalam kontainer canvas agar tidak melompat keluar kartu */}
                    {activePoint && (
                        <div
                            className="absolute z-20 pointer-events-none transition-all duration-150 ease-out bg-[#1F6F5F] text-white border border-white/20 shadow-xl rounded-2xl p-3.5 text-xs w-60 sm:w-64"
                            style={{
                                top: '16px',
                                left: `${(activePoint.x / width) * 100}%`,
                                transform:
                                    hoveredIdx !== null && hoveredIdx >= 3
                                        ? 'translateX(calc(-100% - 14px))'
                                        : 'translateX(14px)',
                            }}
                        >
                            <div className="flex items-center justify-between border-b border-white/20 pb-1.5 mb-2">
                                <span className="font-heading font-serif font-bold text-sm text-[#6FCF97]">
                                ...
                                {showHotspots && (
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-bold text-white">
                                            {activePoint.data.hotspots.toLocaleString()} titik
                                        </span>
                                    </div>
                                )}
                                {showBurnedArea && (
                    ...
                    >
                        <defs>
                            <linearGradient id="ispaGrad" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="0%" stopColor="#2FA084" stopOpacity="0.30" />
            ...
            <div className="mt-6 rounded-2xl border border-[#EEEEEE] bg-white dark:bg-card p-5 shadow-xs">
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                    <div className="p-3.5 rounded-xl bg-[#EEEEEE]/60 border border-[#EEEEEE]">
